    ...
    Cardboard.SDK.UpdateState();
			...
			//Debug.Log(Input.acceleration.x);
								...
								if (target == null) {
										...
										transform.localRotation = new Quaternion (0f, 0f, 1.5f*Mathf.Clamp (rot.z, -0.4f, 0.4f), rot.w);
			...
			transform.parent.position = new Vector3(Mathf.Clamp(transform.parent.position.x+speed*Time.deltaTime*(-1f)*transform.localRotation.z,-58f,+58f),transform.parent.position.y,transform.parent.position.z);


		//trackposition is set to false here!
    if (trackPosition) {
